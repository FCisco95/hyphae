...
**Shipped:** the workspace. pnpm monorepo, Anchor 1.0.1 program at the root that builds and emits its IDL, Biome for lint, LF enforced. WSL toolchain complete (Node 22, pnpm 10, Rust 1.90, Anchor 1.0.1, solana-cli 3.1). Fly app `hyphae-api` created.
**Decisions:**
...
**Next:** accounts (Neon, BotFather, model keys), then `packages/core` merkle test-first.
