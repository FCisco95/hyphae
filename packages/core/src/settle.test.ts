...
const ws = (wallet: string, scoreSum: number, stakeUnits = 0n): WalletScore => ({ wallet, scoreSum, stakeUnits });

describe("settle", () => {
  it("splits the pot proportionally to score when stakeWeight is none", () => {
    const out = settle([ws("a", 30), ws("b", 70)], { stakeWeight: "none", potLamports: 1_000_000n });
  ...
  it("never pays out more than the pot (rounding goes to the vault)", () => {
    const out = settle([ws("a", 1), ws("b", 1), ws("c", 1)], { stakeWeight: "none", potLamports: 100n });
  ...
  it("sqrt stake weighting is concave", () => {
    const out = settle([ws("a", 10, 100n), ws("b", 10, 10_000n)], { stakeWeight: "sqrt", potLamports: 1_100n });
  ...
  it("applies the per-wallet cap and does not redistribute the remainder", () => {
    const out = settle([ws("a", 90), ws("b", 10)], { stakeWeight: "none", potLamports: 1_000n, capLamports: 500n });
